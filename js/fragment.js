...
//    }
  ...
  var _load = function(url, callback) {
    ...
    if (_cache) {
      ...
      else callback(_cache);
    } else {
      _cache_load[url] = 1;
      load(url, function(_newcache) {
        _cache_load[url] = _newcache;
        callback(_newcache);
      });
    }
  };
  ...
  var render_fragment = function(fragment_type, element) {
    ...
    if (fragment_type.html && fragment_type.json) {
      _load(html_url, function(html) {
        _load(json_url, function(json) {
